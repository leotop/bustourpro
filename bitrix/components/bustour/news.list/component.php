    ...
    if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
    {
        ...
        if($arResult = $rsIBlock->GetNext())
        {
            ...
            $arFilter = array (
                ...
                "ACTIVE" => "Y",
                "CHECK_PERMISSIONS" => "Y",
                "PROPERTY_COMPANY" => getCurrentCompanyID(), //фильтруем по компании    
                ">PROPERTY_DATE_FROM" => date("Y-m-d")." 23:59:00", //фильтруем по дате
                ">PROPERTY_NUMBER_ROOM" => 0
            );

            //если установлен фильтр
            if ($_GET["arrival_date_begin"] !="") {
                $arDay = explode(".",$_GET["arrival_date_begin"]);
                $arFilter[">=PROPERTY_DATE_FROM"] = $arDay[2]."-".$arDay[1]."-".$arDay[0]." 00:00:00";            
            }

            if ($_GET["arrival_date_end"] !="") {
                $arDay = explode(".",$_GET["arrival_date_end"]);
                $arFilter["<=PROPERTY_DATE_FROM"] = $arDay[2]."-".$arDay[1]."-".($arDay[0])." 23:59:00";            
            }  

            if ($_GET["city"] && $_GET["city"][0] !=0) {
                $arFilter["PROPERTY_CITY"]= $_GET["city"]; 
            }


            if ($_GET["hotel"]&& $_GET["hotel"][0] !=0) {
                $arFilter["PROPERTY_HOTEL"]= $_GET["hotel"]; 
            }

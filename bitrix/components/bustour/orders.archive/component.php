<?
    ...
    if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
    {
        ...
        if($arResult = $rsIBlock->GetNext())
        {
            ...
            $days_count = CIBlockElement::GetList(array(),array("CODE"=>"DAYS_COUNT_FOR_ARCHIVE","PROPERTY_COMPANY"=>getCurrentCompanyID()), false, false, array("PROPERTY_VALUE"));
            $arDaysCount = $days_count->Fetch();
            
            //получаем метку времени для фильтрации
            $cur_day_label = date("U");
            $filter_date_label = $cur_day_label - $arDaysCount["PROPERTY_VALUE_VALUE"]*86400;

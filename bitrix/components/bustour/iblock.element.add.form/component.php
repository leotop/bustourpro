...
if (CModule::IncludeModule("iblock"))
{
    ...
    if ($bAllowAccess)
    {
        ...
        while ($arProperty = $rsIBLockPropertyList->GetNext())
        {
            ...
            if ($arProperty["PROPERTY_TYPE"] == "E"/* && $arProperty['CODE'] == "COMPANY"*/)
            {
                $arFilter = array(
                ...
                $adEl = CIBlockElement::GetList(array("SORT" => "asc", "NAME" => "asc"), $arFilter);

                while ($el = $adEl->GetNextElement()) {

                    $arPropertyEnum = $el->GetFields();
                    $arProps = $el->GetProperties();
                    //__dump($arProps, true);
                    $arProperty["ENUM"][$arPropertyEnum["ID"]] = array(
                        "ID" => $arPropertyEnum["ID"],
                        "PROPERTY_ID" => $arProperty["ID"],
                        "VALUE" => $arPropertyEnum["NAME"],
                        "DEF" => "N",
                    );
                }
            ...
            //Replace CODE on ID
            if (($key = array_search($arProperty["CODE"], $arParams["PROPERTY_CODES"])) !== false) {
                $arParams["PROPERTY_CODES"][$key] = $arProperty["ID"];
            }
            if (($key = array_search($arProperty["CODE"], $arParams["PROPERTY_CODES_REQUIRED"])) !== false) {
                $arParams["PROPERTY_CODES_REQUIRED"][$key] = $arProperty["ID"];
            }
    ...
    //__dump($arResult["PROPERTY_LIST_FULL"], true);
    if ($bAllowAccess)
    {
        // process POST data
        if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
        {
                    ...
                    if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "F")
                    {
                            ...
                            else
                            {
                                foreach ($arPropertyValue as $key => $value)
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        if (empty($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"][$value])) {
                                            $value = null;
                                        }
                                    ...
                                    /*if (
                                        $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "E" && intval($value) > 0
                                        ||
                                        $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "E" && !empty($value)
                                    )
                                    {
                                        $arUpdatePropertyValues[$propertyID][] = $value;
                                    }*/
                        ...
                        else
                        {
                            ...
                            {  
                                ...
                                if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "N") {

                                    if (!is_numeric($arPropertyValue[0])) {
                                        $arResult["ERRORSS"][] = "Поле '". $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ."' должны быть числом.";
                                    }
                                }
            ...
            if (count($arResult["ERRORS"]) == 0)
            {
                ...
                if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID") {
                    ...
                    //При сохранении не меняем
                    //$arUpdatePropertyValues[$arParams["ELEMENT_ASSOC_PROPERTY"]] = getCurrentCompanyID();
                }

                //$arUpdatePropertyValues[$arParams["ELEMENT_ASSOC_PROPERTY"]] = getCurrentCompanyID();
                //$arUpdatePropertyValues["COMPANY.ID"] = getCurrentCompanyID();
                ...
                //__dump($arUpdateValues, true);
               // $arUpdateValues["PROPERTY_COMPANY_ID"] = getCurrentCompanyID();
                ...
                else
                {
                    ...
                    $arUpdateValues["PROPERTY_VALUES"]["COMPANY"] = getCurrentCompanyID();
                    //__dump($arUpdateValues);die;
            ...
            foreach ($arResult["PROPERTY_LIST"] as $propertyID)
            {
                ...
                if ($arProperty["PROPERTY_TYPE"] == "F")
                {
                    ...
                    foreach ($arValues as $value)
                    {
                        ...
                        {
                            $arFile["PATH_IMAGE"] = $arFile["FILE_NAME"];
                            $arFile["IS_IMAGE"] = CFile::IsImage($arFile["FILE_NAME"], $arFile["CONTENT_TYPE"]);
                            if ($arFile["IS_IMAGE"]) {
                                CFile::ResizeImage($arFile["FILE_NAME"], array("width" => "1200", "height" => "1200"), BX_RESIZE_IMAGE_PROPORTIONAL);
                            }
        ...
        if ($bShowForm)
        {
            ...
            if (count($arResult["ERRORS"]) > 0)
            {
                ...
                //echo "<pre>",htmlspecialcharsbx(print_r($arUpdatePropertyValues, true)),"</pre>";

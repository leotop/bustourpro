    ...
    if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
    {
        ...
        if($arResult = $rsIBlock->GetNext())
        {
            ...
            if ($_GET["set_filter"] == "Y") {
                ...
                //дата отправления ОТ
                if ($_GET["departure_date_begin"]) {
                    $departureFrom = explode(".",$_GET["departure_date_begin"]);
                    if (mktime(0,0,0,$departureFrom[1],$departureFrom[0],$departureFrom[2]) < date("U")) {
                        $departureFrom = explode(".",date("d.m.Y"));  
                    }
                    $arrFilter[">=PROPERTY_DEPARTURE"] = $departureFrom[2]."-".$departureFrom[1]."-".$departureFrom[0]." 00:00:00";  
                }  
                //дата отправления ДО
                if ($_GET["departure_date_end"]) {
                    $departureTo = explode(".",$_GET["departure_date_end"]);
                    if (mktime(0,0,0,$departureTo[1],$departureTo[0],$departureTo[2]) < date("U")) {
                        $departureTo = explode(".",date("d.m.Y"));  
                    }
                    $arrFilter["<=PROPERTY_DEPARTURE"] = $departureTo[2]."-".$departureTo[1]."-".$departureTo[0]." 23:59:59";  
                } 


                //дата отправления ОТ
                if ($_GET["arrival_date_begin"]) {
                    $arrivalFrom = explode(".",$_GET["arrival_date_begin"]);
                    if (mktime(0,0,0,$arrivalFrom[1],$arrivalFrom[0],$arrivalFrom[2]) < date("U")) {
                        $arrivalFrom = explode(".",date("d.m.Y"));  
                    }
                    $arrFilter[">=PROPERTY_ARRIVAL"] = $arrivalFrom[2]."-".$arrivalFrom[1]."-".$arrivalFrom[0]." 00:00:00";  
                }  
                //дата отправления ДО
                if ($_GET["arrival_date_end"]) {
                    $arrivalTo = explode(".",$_GET["arrival_date_end"]);
                    if (mktime(0,0,0,$arrivalTo[1],$arrivalTo[0],$arrivalTo[2]) < date("U")) {
                        $arrivalTo = explode(".",date("d.m.Y"));  
                    }
                    $arrFilter["<=PROPERTY_ARRIVAL"] = $arrivalTo[2]."-".$arrivalTo[1]."-".$arrivalTo[0]." 23:59:59";  
                }


                if ($_GET["bus_direction"] > 0) {
                    $arrFilter["PROPERTY_BUS_DIRECTION"] = $_GET["bus_direction"]; 
                }
                ...
                $arrFilter[">=PROPERTY_DEPARTURE"] = date("Y-m-d 00:00:00"); 
            }
            ...
            */
            ...
            //arshow(array_merge($arFilter, $arrFilter));

            $arSort["PROPERTY_DEPARTURE"] = "ASC";

            $obParser = new CTextParser;
            $arResult["ITEMS"] = array();
            $arResult["ELEMENTS"] = array();
            $rsElement = CIBlockElement::GetList($arSort, array_merge($arFilter, $arrFilter), false, $arNavParams, array_merge($arSelect,array("PROPERTY_ARRIVAL","PROPERTY_DEPARTURE")));

...
<a href="/"><?=GetMessage("SUP_BACK")?></a>
<br>
<br>
<h5><?=GetMessage("SUP_TICKET")?> π<?=$arResult["TICKET"]["ID"]?>. <span style="text-transform: lowercase;"><?=$arResult["TICKET"]["TITLE"]?></span></h5>
            ...
            <tr>
                <td>
                    ...
                    <?if ($arResult["USER"]["IS_STAFF"] == "Y"){?>
                        ...
                        <br><br> 
                        <label><input type="checkbox" id="UF_IN_PAYMENT" name="UF_IN_PAYMENT" value="Y" <?if ($arResult["TICKET"]["IN_PAYMENT"] == "Y" || !$arResult["TICKET"]["IN_PAYMENT"]){?> checked="checked"<?}?>> ”◊¿—“¬”≈“ ¬ ŒœÀ¿“≈</label>
        ...
        <script type="text/javascript">
            ...
            <? if($_REQUEST["NEW_TICKET"]==1) {?>
                ...
                function check_status() {  
                    ...
                    if (commentsRequire[$("#STATUS_SID").val()] == "Y" && $("#MESSAGE").val()=="" && ($("#CLOSE").length > 0 && !$("#CLOSE").prop("checked")) && ($("#curStatus").val() != $("#STATUS_SID").val())) {

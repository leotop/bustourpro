...
<script>
    ...
    $(function(){
        <?
            //проверяем - если город по умолчанию уже выбран, то текущий город нельзя сделать городом по умолчанию
            $d_city_default = CIBLockElement::GetList(array(), array("IBLOCK_CODE"=>"DEPARTURE_CITY","PROPERTY_DEFAULT_VALUE"=>"Да","PROPERTY_COMPANY"=>getCurrentCompanyID()), false, false, array("NAME","ID"));
            $count = $d_city_default->SelectedRowsCount();
            $arD_city_default = $d_city_default->Fetch();
            if ($count > 0 && $arD_city_default["ID"] != $arResult["ELEMENT"]["ID"]) {                  
            ?>
            $("#property_DEFAULT").attr("disabled","disabled");
            $("#property_DEFAULT").attr("title","В данный момент городом по умолчанию является <?=$arD_city_default["NAME"]?>");
            $("#property_DEFAULT").siblings("label").attr("title","В данный момент городом по умолчанию является <?=$arD_city_default["NAME"]?>");
            $("#property_DEFAULT").siblings("label").css("color","#ddd");
            <?}?>
    })

...
<script>
    ...
    //проверяем выбран ли чекбокс "возможность заселения с детьми"
    function children_check(){    
        if ($("#property_IS_CHILDREN").attr("checked") == "checked") {
            $("#property_AGE_CHILDREN").removeAttr("disabled");
            $("#property_AGE_CHILDREN").siblings("label").removeAttr("style");
        }
        else {
            $("#property_AGE_CHILDREN").attr("disabled","disabled"); 
            $("#property_AGE_CHILDREN").val("");
            $("#property_AGE_CHILDREN").siblings("label").css("color","#DDDDDD");
        }
    }
    ...
    function select_from_filter(prop_name,value) {
     ...
     //проверка свойства 1человек/кровать
    function oneManCheck() {
        if ($("#property_ONE_MAN_FOR_BED").attr("checked") == "checked") {
           $("#property_COST_UTILITIES_SERVICE").val("");
           $("#property_COST_UTILITIES_SERVICE").attr("disabled","disabled"); 
        }
        else {
           $("#property_COST_UTILITIES_SERVICE").removeAttr("disabled","disabled"); 
        }
    }
    ...
    $(function(){
        ...
        //клик по чекбоксу "возможность заселения с детьми"
        $("#property_IS_CHILDREN").change(function(){
            children_check(); 
        }) 
        ...
        $("#property_ONE_MAN_FOR_BED").change(function(){
           oneManCheck();  
        })
                    ...
                    <?
                        ...
                        case "L":
                            ...
                            case "radio":
                                ...
                                <div class="<?=$type?>">
                                    <label>
                                    <input type="<?=$type?>" id="property_<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"]?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>">
                                        <?=$arEnum["VALUE"]?>
                                    </label>
                                </div>

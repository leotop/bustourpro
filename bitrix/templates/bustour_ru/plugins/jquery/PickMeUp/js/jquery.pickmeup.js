...
//чтобы текущий месяц был первым слева нужно везде где есть код: options.calendars / 2   -  заменить 2 на 6 
...
(function ($) {
    ...
    //собираем массив месяцев в таком порядке, чтобы текущий месяц был последним, тогда при выводе календарь будет начинаться с него    
    var now = new Date();
    var cur_month = now.getMonth(); 
    var months_list_full = [];  //пересобранный массив месяцев (полное название)
    var months_list_short = []; //пересобранный массив месяцев (короткое название)
    var months_full = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"];
    var months_short = ["Янв", "Фев", "Мар", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Ноя", "Дек"];
    for (var i = cur_month+1, k=0; i < cur_month+13; i++, k++) {
        var n = i;
        if (n > 11) {n = n-12;}  
        months_list_full[k] = months_full[n];
        months_list_short[k] = months_short[n]; 
    }
    ...
    var pickmeup = function () {
        ...
        function fill (cal) {
            ...
            var current_cal    = Math.floor(options.calendars / 6),
            date,
            data,
            header,
            year,
        ...
        function click (e) {
            ...
            if (el.hasClass('pmu-button')) {
                ...
                var instance_index    = $('.pmu-instance', root).index(instance);   //порядковый номер календаря (0-4)
                var current_date    = new Date(options.current);
                var val;
                if (el.parent().is('nav')) {
                    if (el.hasClass('pmu-month')) {
                        current_date.addMonths(instance_index - Math.floor(options.calendars / 6));
                    ...
                    if (root.hasClass('pmu-view-years')) {
                        ...
                        options.current.addMonths(Math.floor(options.calendars / 6) - instance_index);
